...
<div class="ins-card relative items-center inline-flex flex-row flex-wrap justify-start p-2 border-y" id="insCard">
    <div class="ins-card__cover">
        {#if data.cover_id}
            <img class="ins-card__cover-img w-16 h-20 aspect-auto" src="{ol_cover_url}{data.cover_id}-M.jpg" alt="book cover">
        {:else}
            <img class="ins-card__cover-img w-16 h-20 aspect-auto" src="{ol_error_url_sm}" alt="book cover">
        {/if}
    </div>
    <div class="text-info break-words max-w-lg font-body">
        <div class="ins-card__title font-body text-lg font-medium pl-2 max-w-lg">{data.title}</div>
    ...
    <div class="date">
        <div class="ins-card__date font-body text-sm pl-2 -translate-y-2">
            {date}
        </div>
    </div>
</div>